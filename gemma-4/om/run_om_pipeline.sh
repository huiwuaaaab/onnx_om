#!/usr/bin/env bash
# =============================================================================
# Gemma-4 OM pipeline (main + assistant speculative decode)
#
# [MDC 板端] 本脚本仅在 MDC 上运行 msame OM 推理。
# 静态 input bin 请在本机用 dump_om_inputs.py 生成后 scp 过来；
# 解析文本请在本机用 parse_state.py 读取 om_output/。
#
# Main:  vision -> mm_proj -> llm_preblock -> b1..b7 -> lm_head
# Assist: assistant OM inputs from main b7/b3 + state
#
# Static bins: dump/vision, dump/llm_preblock/; PLE: ple_table/ (与 dump/ 同级)
# Final output: ${OUTPUT_ROOT}/final_*.bin (+ work/, state/ 子目录)
# All runtime under OUTPUT_ROOT (default om/om_output/):
#   om_output/work/    scratch (default跑完删)
#   om_output/state/   decode 状态
#   om_output/final_*  最终 bin
#
# Auto-detect single vs batch from optional path argument:
#   (no arg)              single — dump/ + om_output/
#   <batch_root>/         batch  — if any <batch_root>/<stem>/dump/vision/ exists
#   <item_dir>/           single — if <item_dir>/dump/vision/ exists
#   <dump_root>/          single — if <dump_root>/vision/ exists
#
# Usage (defaults: MODE=full, GEN_STEPS=50, NUM_ASSISTANT_TOKENS=6, WITH_ASSISTANT=1):
#   bash run_om_pipeline.sh
#   RUN_MSAME=1 bash run_om_pipeline.sh --dump-dir /path/to/dump
#   RUN_MSAME=1 bash run_om_pipeline.sh --dump-dir ./dump --output-dir ./out
#   RUN_MSAME=1 bash run_om_pipeline.sh --batch-root ./batch
#   RUN_MSAME=1 bash run_om_pipeline.sh ./batch
#   RUN_MSAME=1 bash run_om_pipeline.sh ./batch 100 8
#
# Paths (env or flags; --output-dir 为统一输出根目录):
#   DUMP_ROOT / --dump-dir       static input bin root (vision/, llm_preblock/)
#   OUTPUT_ROOT / --output-dir   统一输出根（含 work/ state/ final_*）
#   WORK_ROOT / --work-dir       默认 <output-dir>/work
#   STATE_DIR / --state-dir      默认 <output-dir>/state
#   PLE_TABLE_DIR / --ple-table-dir
#
# Env:
#   MODE, GEN_STEPS, NUM_ASSISTANT_TOKENS, WITH_ASSISTANT, KEEP_INTERMEDIATE
#   DO_PARSE=1            [不推荐] MDC 上顺带 parse；推荐拷回本地 parse_state.py
#   SKIP_EXIST=1          batch: skip item if om_output/work/step_0000 exists
#   RUN_MSAME=0           dry-run (default); set 1 on board
# =============================================================================
...
Specify static input bins (single run):
  --dump-dir PATH, --dump-root PATH   input bin root (contains vision/, llm_preblock/)
...
resolve_om_assistant() {
  ...
  local f
  shopt -s nullglob
  for f in "${OM_DIR}"/assistant*.om; do [[ -f "${f}" ]] && found+=("${f}"); done
  shopt -u nullglob
  if [[ ${#found[@]} -eq 1 ]]; then echo "${found[0]}"; return 0; fi
  echo "ERROR: assistant OM not found under ${OM_DIR}" >&2
  return 1
}

check_dump() {
  for f in "${DUMP_VISION}/pixel_values.bin" "${DUMP_VISION}/image_position_ids.bin" \
           "${DUMP_PREBLOCK}/input_ids.bin" "${DUMP_PREBLOCK}/attention_mask.bin" \
           "${DUMP_PREBLOCK}/per_layer_inputs.bin" "${DUMP_PREBLOCK}/position_ids.bin"; do
...
run_speculative_decode_loop() {
  ...
  for ((step = 0; step < GEN_STEPS; step++)); do
    ...
    run_py_assistant process-speculative-accept \
      --state-dir "${state_dir}" \
      --cur-len "${cur_len}" \
      --candidates-file "$(cat "${WORK_ROOT}/${tag}/assistant_candidates.path")" \
...
EOF
  ...
  for item_dir in "${item_dirs[@]}"; do
    ...
    if [[ ! -f "${dump_dir}/vision/pixel_values.bin" ]]; then
      log "[${idx}/${total}] SKIP ${stem}: missing ${dump_dir}/vision/pixel_values.bin"
...
validate_dump_root() {
  local root="$1"
  [[ -f "${root}/vision/pixel_values.bin" ]] || {
    echo "ERROR: missing ${root}/vision/pixel_values.bin" >&2
    exit 1
  }
}
...
dispatch() {
  ...
  if [[ -f "${input}/dump/vision/pixel_values.bin" ]]; then
    ...
    echo "  expected ${input}/dump/vision/pixel_values.bin (item dir)" >&2
    echo "  or ${input}/vision/pixel_values.bin (dump root)" >&2
    echo "  or ${input}/<stem>/dump/vision/... (batch root)" >&2
    echo "  or use: --dump-dir /path/to/dump" >&2
    exit 1
  fi
