...
while [[ ! -f "${QUEUE_DIR}/exit" ]]; do
  ...
  if [[ "${RUN_MSAME}" == "1" ]]; then
    ...
    python3 - "${WORKER_NAME}" "${OUTPUT_DIR}" <<'PY'
import sys, pathlib
worker, out_s = sys.argv[1], sys.argv[2]
