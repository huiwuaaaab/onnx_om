#!/usr/bin/env bash
# =============================================================================
# InternVL3_5-1B OM pipeline
#
# [MDC] vision_448 -> mm_proj -> llm_preblock -> b1..b3 -> lm_head
...
Env: MODE=prefill_only|full|decode  RUN_MSAME=1  KEEP_INTERMEDIATE=0|1
EOF
}
...
run_vision_mmproj() {
  ...
  run_om_model "${prefix}/vision_448" "${OM_VISION}" "${vision_in}" "${vision_out}" 1
...
main() {
  ...
  OM_VISION="$(resolve_om "${OM_VISION:-}" "vision_448" "vision_448")" || exit 1
