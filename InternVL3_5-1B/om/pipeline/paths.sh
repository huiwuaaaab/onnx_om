#!/usr/bin/env bash
# Shared static input path resolution for InternVL3_5-1B OM pipeline scripts.
#
# New layout (dump_vision_om_inputs.py + dump_llm_preblock_inputs.py):
#   single:  om/vision_bin/  +  om/prompt_bin/
#   batch:   batch/<stem>/vision_bin/  +  shared om/prompt_bin/ (or per-item prompt_bin/)
#
# Legacy layout still supported:
#   dump/vision/  +  dump/llm_preblock/
...
# stdout: vision bin directory containing pixel_values.bin
resolve_vision_bin() {
  local base="$1"
  [[ -d "${base}" ]] || return 1
  if [[ -f "${base}/pixel_values.bin" ]]; then
    echo "${base}"; return 0
  fi
  if [[ -f "${base}/vision_bin/pixel_values.bin" ]]; then
    echo "${base}/vision_bin"; return 0
  fi
  if [[ -f "${base}/vision/pixel_values.bin" ]]; then
    echo "${base}/vision"; return 0
  fi
  if [[ -f "${base}/dump/vision/pixel_values.bin" ]]; then
    echo "${base}/dump/vision"; return 0
  fi
  return 1
}

# stdout: preblock bin directory containing input_ids.bin
resolve_prompt_bin() {
  local base="${1:-}"
...
configure_default_single_inputs() {
  ...
  VISION_BIN="$(resolve_vision_bin "${root}")" || {
    echo "ERROR: missing ${root}/vision_bin/pixel_values.bin" >&2
...
# stdout: human-readable reason when item inputs are incomplete
diagnose_item_inputs() {
  ...
  if resolve_vision_bin "${item_dir}" &>/dev/null; then
    ...
    v_paths="${item_dir}/vision_bin/pixel_values.bin"
...
configure_item_inputs() {
  ...
  PROMPT_BIN="$(resolve_prompt_bin "${item_dir}")" || {
    echo "ERROR: missing prompt_bin for ${item_dir} (and no shared $(om_paths_shared_prompt_bin))" >&2
    diagnose_item_inputs "${item_dir}" >&2
    return 1
  }
  INPUT_ROOT="${item_dir}"
