...
# Multi-image OM stage pipeline: vision/mm_proj/preblock/block* workers run in parallel.
...
declare -a STAGE_NAMES=(vision mm_proj preblock block1 block2 block3 lm_head)
declare -a STAGE_WORKERS=(vision mm_proj preblock block1 block2 block3 lm_head)
declare -a STAGE_NUM_INS=(1 1 4 4 4 4 1)
declare -a OM_WORKER_NAMES=(vision mm_proj preblock block1 block2 block3 lm_head)
...
Env: MODE=prefill_only|full|decode  RUN_MSAME=1  SHARED_PROMPT_BIN=om/prompt_bin

Pipeline: up to 7 images in-flight (one per OM stage worker).
EOF
}
...
start_om_workers() {
  ...
  if [[ "${OM_RESIDENT:-0}" == "1" ]]; then
    if [[ "${OM_WORKER_SH##*/}" == "worker_cpp_resident.sh" ]]; then
      log "pipe workers up (7 cpp resident)  root=${PIPE_ROOT}"
    else
      log "pipe workers up (7 resident)  root=${PIPE_ROOT}"
    fi
  else
    log "pipe workers up (7 processes)  root=${PIPE_ROOT}"
  fi
}
...
pipe_prep_stage() {
  local idx="$1" stage="$2"
  local base tag vision_out mmproj_out pre_out prev_out bi
  base="$(img_step_base "${idx}")"
...
pipe_submit_stage() {
  ...
  case "${stage}" in
    ...
    3|4|5)
      ...
      ;;
    6) input_dir="${base}/lm_head_in"; output_dir="${base}/lm_head_out" ;;
  esac
